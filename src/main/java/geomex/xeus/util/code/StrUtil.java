...
public class StrUtil {
    ...
    /**
     * 넘어온 json 값에 오브젝트가 있는지 체크하고 key 값을 전부 소문자로 변경한다.
     *  - NDMS 용으로 커스텀한다. ( 세종시 스마트 재난정보 전파 시스템 )
     *    * AWS 중 같은 읍면동으로 넘어올 시 ( 제거 )
     * @param list
     * @return
     */
   	public static List<HashMap<Object, Object>> keyChangeLower(List<HashMap<Object, Object>> list, int code) {

   		ArrayList<HashMap<Object, Object>> newList = new ArrayList<HashMap<Object, Object>>();
    	...
    	for ( int i = 0; i < list.size(); i++ ) {
        	...
        	for (Object key : map.keySet()) {
        		...
        		if (map.get(key).getClass() != JSONObject.class) {
        			String v = map.get(key).toString();

        			if ( v.trim().equals("")) v = null;
        			tmpMap.put(key.toString().toLowerCase(), v);

        		} else {
        			tmpMap.put(key.toString().toLowerCase(), null);
        		}

        	}

        	if ( code == 7 ){
        		if ( !removeList.contains(tmpMap.get("userdefineareanm")+";"+tmpMap.get("dtobz")) ) {

        			removeList.add(tmpMap.get("userdefineareanm").toString()+";"+tmpMap.get("dtobz"));
        			newList.add(tmpMap);
        		}
        	} else {
