...
_common = {
	...
	callAjax : function(_URL, _PARAMETERS, _CALLBACK){
		...
		if(_URL != null){
			...
			var _isView = false;
			if(_URL.endsWith(".do")) _isView = true;
			if(_isView){
					...
					if(json.error != null){
						...
						if("code" in json){
							if(json.code == "GMX-1001"){
								if(sysConfirm){
									if(sysConfirm("이미 접속중인 사용자 입니다.\n\n접속일) " + json.signDate + "\n\n강제 로그아웃 처리 하시겠습니까?")){
										_common.callAjax("/user/sessionDestroy.json", { "user" : json.userId }, function(res){
											if(res.result){
												alert("강제 로그아웃 되었습니다.");
												$("#login_btn").click();
											}else{
												alert("강제 로그아웃을 실패하였습니다.");
											}
										}, false);
									}
								}else{
									sysConfirm("이미 접속중인 사용자 입니다.\n\n접속일) " + json.signDate + "\n\n강제 로그아웃 처리 하시겠습니까?", function(){
										_common.callAjax("/user/sessionDestroy.json", { "user" : json.userId }, function(res){
											if(res.result){
												alert("강제 로그아웃 되었습니다.");
												$("#login_btn").click();
	 ...
	 * </pre>
	 */
	callLoadingAjax : function(_URL, _PARAMETERS, _CALLBACK){
	...
	getColumn : function(_Param, _Type){
		...
		if(_Param != null){
			...
			if(typeof _Param == "string"){
				this.callAjax("/column/getColumn.json", { "tbl" : _Param, "dataType" : _Type }, function(json){
					list = json.result;
				}, false);
			}
	...
	postForm : {
		...
		submit : function(_URL, _PARAMETERS, _HREF){
			...
			if(_URL != null){
				if($(parent)[0].parentWindow) $(parent)[0].parentWindow.onbeforeunload = null;
				...
				setTimeout(function(){
					if($(parent)[0].parentWindow) $(parent)[0].parentWindow.onbeforeunload =  function(){ return "" };
				}, 1000);
			}else{
				alert("올바른 요청이 아닙니다.");
			}
	...
	collectSendData : function(){
		...
		if(_common.utils.validObject(target, "string")){
			$(target).find(".sendData").each(function(){
				if(!_common.utils.isNullAndEmpty($(this).attr("id"))){
