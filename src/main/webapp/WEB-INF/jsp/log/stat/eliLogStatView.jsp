...
<%@ page import="geomex.xeus.eventmonitor.service.EliLogStatVo"%>
...
<%
ArrayList<EliLogStatVo> listeli = (ArrayList<EliLogStatVo>) request.getAttribute("eli");
...
$("#wrap").find("button.logTab").click(function(){
	if($(this).attr("active") != "active"){
		...
		if(url != null){
			var _param = {};
			_param['year'] = 2018;//$('#wrap').find('#search').find('#year').val()
			console.log(_param);
...
<div>
    <div id="wrap">
        <p class="searchTitle">
            ...
            <button class="logTab" active="active" url="/event/getEliLogStatView.do">긴급재난상황</button>
            <button class="logTab" url="/log/getEs112LogStatView.do">112 긴급출동</button>
        </p>
        <div id="title">긴급재난상황 통계</div>
        <div id="search">
            <span>년도 : </span>
            <select id="year">
...
<%
    ...
    for(int i=0; i<listeli.size(); i++){
%>
                    <tr>
                        <td><%= listeli.get(i).getGbn() %></td>
                        <td><%= listeli.get(i).getAll() %></td>
                        <td><%= listeli.get(i).getJan() %></td>
                        <td><%= listeli.get(i).getFeb() %></td>
                        <td><%= listeli.get(i).getMar() %></td>
                        <td><%= listeli.get(i).getApr() %></td>
                        <td><%= listeli.get(i).getMay() %></td>
                        <td><%= listeli.get(i).getJun() %></td>
                        <td><%= listeli.get(i).getJul() %></td>
                        <td><%= listeli.get(i).getAug() %></td>
                        <td><%= listeli.get(i).getSep() %></td>
                        <td><%= listeli.get(i).getOct() %></td>
                        <td><%= listeli.get(i).getNov() %></td>
                        <td><%= listeli.get(i).getDec() %></td>
                    </tr>
<%
    }
}
