(function(){
	...
	$("#" + parentView).find("#unregVoice").click(function(){
		...
		_common.callAjax("/voice/getUnregVoiceView.do", null, function(view) {
		...
		if(nm != ""){
			confirm("선택하신 파일을 업로드 하시겠습니까?", function(){
	            ...
	            _common.formSubmit("/image/add.json", $("#" + parentView).find("#hiddenForm"), function(json){
	                if(json.result){
	                    _common.callAjax("/voice/getVoiceMngView.do", {mgrNo : k}, function(view) {
	        				$("#" + parentView).find("#center-overlay-east").height(
	        					$(window).height() - $("#" + parentView).find("#layout-north").outerHeight() - $("#" + parentView).find("#overlay-east-bar").outerHeight()
	        				).html(view);
...
//		console.log("mgr3[1] = "+mgr3[1]);
		...
		param["tmLat"]=lat;
		param["tmLon"]=lng;
		console.log("tmlat = "+param["tmLat"]);
		console.log("tmlon = "+param["tmLon"]);
		var first=
		{
		...
		confirm("수정하시겠습니까?", function(){
			_common.callAjax("/voice/edit.json", param, function(json) {
				if(json){
					alert("저장되었습니다.");

					Layers['asset_eqb_voice'].reload();
...
//		var k = $(this).parent().find("#k").val();	//mgrNo
		...
		confirm("삭제하시겠습니까?", function(){
			_common.callAjax("/voice/del.json", {mgrNo : v}, function(json) {
				if(json){
					alert("삭제되었습니다.");
				    	...
				    	_common.callAjax("/image/del.json", {mgrNo : v}, function(json){
				            if(json.result){
								...
								$("#" + parentView).find(".searchWrapper").find("#objType").val("VOI");
