...
public class SituationVo {
    ...
    public String json(){
        ...
        json.element("RECEPT_YMD", RECEPT_YMD);
        json.element("EVT_OCR_NO", EVT_OCR_NO);
        json.element("MTMDA_PRCS_STATE", MTMDA_PRCS_STATE);
        json.element("TITLE", TITLE);
        json.element("IMG_START_YMD_HMS", IMG_START_YMD_HMS);
        json.element("IMG_END_YMD_HMS", IMG_END_YMD_HMS);
        json.element("POINT_X", POINT_X);
        json.element("POINT_Y", POINT_Y);
        json.element("ADRES_NM", ADRES_NM);
        json.element("CONTS", CONTS);
        json.element("RECEPT_NM", RECEPT_NM);
        json.element("RECEPT_TELNO", RECEPT_TELNO);
        json.element("imageSetCnt", imageSetCnt);
        
        JSONArray array = new JSONArray();
        for(int x = 0 ;x < imageSetCnt; x++){
            array.add(imageSet.get(x).json()); 
        }
        json.element("imageSet", array);
