	...
	_common.callAjax("/eventHist/getStatList.json", _param, function(json){
		...
		if(json.count == 0){
			...
			for(var i=0; i<json.result.length; i++){
				/* console.log(json.result[i].statEvetNm + ' // ' +json.result[i].outbPosNm + ' // ' +json.result[i].outbPosX
								+ ' // ' +json.result[i].outbPosY + ' // ' +json.result[i].statEvetOutbDtm
								 + ' // ' +json.result[i].procSt); */
				_str += '<tr>';
