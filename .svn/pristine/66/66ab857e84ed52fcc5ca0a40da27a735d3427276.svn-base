...
{
	vo = (VoiceVo) request.getAttribute("vo");
}
...
%>
<script type="text/javascript" src="<%= context %>/res/geomex.xeus.voice.reg.js"></script>

<div id="overlay-east-bar" class="overlay-bar">
    <b class="overlay-bar-title">방송장비 관리</b>
...
<% } %>
		...
		<tr>
			<th>위도</th>
			<td>
				<input type="text" id="lat" name="lat" class="sendData wide loc" value="<%= StrUtil.chkNull(vo.getTmLat()) %>">
			</td>
			<th>경도</th>
			<td>
				<input type="text" id="lng" name="lng" class="sendData wide loc" value="<%= StrUtil.chkNull(vo.getTmLon()) %>">
			</td>
		</tr>
