...
<%@ page import="geomex.xeus.eventmonitor.service.SoundVo"%>
...
<%
    ...
    ArrayList<SoundVo> sound = (ArrayList<SoundVo>)request.getAttribute("sound");

%>
<script type="text/javascript" src="<%=context%>/res/geomex.xeus.event.sound.view.js"></script>
<style>
...
<div class="searchWrapper">
    ...
    <p class="searchTitle">이상음원 모니터링</p>
    <table id="settingTable">
        ...
        %>
        <tr evtno="<%= sound.get(i).getEvtMgrNo() %>">
            <td><%= DateUtil.formatDate( sound.get(i).getRecvDat() ) %></td>
            <td><%= StrUtil.chkNull( sound.get(i).getCctvNm() ) %></td>
            <td>코드표</td>
        </tr>
    ...
    <p class="searchTitle">이상음원 상세정보</p>
    <div>
        <span>종료사유</span>
