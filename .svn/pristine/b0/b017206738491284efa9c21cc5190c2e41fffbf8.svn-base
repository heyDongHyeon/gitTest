...
public class SituationListVo {
    ...
    public String json() {
        ...
        jsonList.element("retCode", retCode);
        jsonList.element("retMsg", retMsg);
        jsonList.element("retYmdHms", retYmdHms);
        jsonList.element("retCnt", retCnt);
        //
        JSONArray array = new JSONArray();
        for (int x = 0; x < retCnt; x++) {
            array.add(retData.get(x).json());
        }
        jsonList.element("retData", array);
